...
def get_message(ch, method, properties, body):
    ...
    content = base64.standard_b64decode(base64.standard_b64decode(body['email'])).decode('utf-8')\
          ...
          .split('</fileContent>')[0]
    print(content)
    results = find_mats.find_mats(content.split('\n'))
...
channel.queue_declare(queue='get_message')
channel.queue_bind(
        exchange='ai', queue='get_message', routing_key='orderrecognition.find_request')

channel.basic_consume(
    queue='get_message', on_message_callback=get_message, auto_ack=True)
