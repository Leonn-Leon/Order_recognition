...
def find_quantities_and_units(text):
    ...
    pattern = r'\b(т|тн|кг|мп|шт|пд|м2)?\s*(-)?\s*(\d+\.?\d*)\s*(-)?\s*(т|тн|кг|мп|шт|пд|м2)?\b'
    ...
    for unit_before, dash1, quantity, dash2, unit_after in matches:
        ...
        if unit_before:
            quantities_and_units.append((quantity, unit_before))
        if unit_after:
            quantities_and_units.append((quantity, unit_after))
