...
class GeminiParser:
    ...
    def parse_order_text(self, text: str) -> list[dict]:
        ...
        VALID_BASE_NAMES = ['арматура', 'уголок', 'труба_круглая', 'труба_профильная', 'лист', 'швеллер', 'круг', 'балка']
        ...
        **КЛЮЧЕВОЕ ПРАВИЛО: Извлекай в `params` только те характеристики, которые ЯВНО УКАЗАНЫ в тексте. Ничего не додумывай.**
        ...
        - `номер`: Для швеллера или балки. Извлекай ПОЛНОЕ значение с буквой (например, "24у", "25к1").
       ...
       - `диаметр`: Для круглых труб, арматуры, кругов. **Ищи его в форматах "ф12", "d12" или "12-А500С".**
        ...
        - `длина`: Длина или тип поставки. Нормализуй числовые значения, убирая "м" или "метров" ("12 метров" -> "12"). Для нечисловых используй "бухта" или "немер". **Не извлекай длину из сложных спецификаций количества вроде `(L=410*...*...шт)`.**
        - `тип`: Только для труб ('вгп', 'оц'). Если типов несколько, верни их в виде списка: ["оц", "вгп"].
